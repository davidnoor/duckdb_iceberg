...
string IcebergSnapshot::GetMetaDataPathFromAWSGlue(const string &path, FileSystem &fs) {
	...
	try {
		...
		return table_metadata_location->second;
...
string IcebergSnapshot::GetMetaDataPath(const string &path, FileSystem &fs, string metadata_compression_codec, string table_version, string version_format) {
	// If the path starts with a { it is presumed to be a JSON encoded representation of a remote
	// iceberg catalog.
	string metadata_file_path;
	if (catalog_type == "glue") {
		metadata_file_path = GetMetaDataPathFromAWSGlue(path, fs);
	} else if (StringUtil::EndsWith(path, ".json")) {
		metadata_file_path = path;
	} else {
		auto meta_path = fs.JoinPath(path, "metadata");
		string version_hint = table_version;
		...
		metadata_file_path = GenerateMetaDataUrl(fs, meta_path, version_hint, metadata_compression_codec, version_format);
	}
